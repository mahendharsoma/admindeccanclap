...
class Vendors_Control extends Default_Controller
{
    ...
    public function index($seg1 = false, $seg2 = false)
	{
        ...
        if(!empty($enc_parent_id) && !empty($parent_id)){
            ...
            $vendor_managers_info = $this->user_library->get_user_details_by_user_roles_and_user_id($user_roles, $parent_id);
            if($vendor_managers_info === false)
            {
                log_message('error', __METHOD__.' error while getting user data for roles: '.implode(', ', $user_roles));
                $this->set_flash_error('Unable to get users, please try later');
                return redirect()->to('dashboard');
            }
            $data['vendor_managers_data'] = $vendor_managers_info;

            $enc_vendor_manager_id = $seg2;
            $vendor_manager_id = get_decoded_value($enc_vendor_manager_id);

            if(!empty($vendor_manager_id) && !empty($vendor_manager_id))
            {

                $data['selected_vendor_manager_id'] = $vendor_manager_id;

                $user_roles = [8];

                $vendors_info = $this->user_library->get_user_details_by_user_roles_and_user_id($user_roles, $vendor_manager_id);
                if($vendors_info === false)
                {
                    log_message('error', __METHOD__.' error while getting user data for roles: '.implode(', ', $user_roles));
                    $this->set_flash_error('Unable to get users, please try later');
                    return redirect()->to('dashboard');
                }
                $data['vendors_data'] = $vendors_info;
            }
        }

	    return $this->generateView('content_panel/User_Management/vendors_view',$data);
	}
    ...
    public function ajax_add_vendor()
    {
       // --------------------- VALIDATION ---------------------
        ...
        $this->ajax_success_response('Successfully Created Vendor!', array(), 'Yes');
        return;
    }

    public function ajax_get_edit_vendor_details()
    {
    ...
    public function ajax_update_vendor()
    {
       $rules = [
        ...
        ---------------------------------------------------------- */
        $this->db->transComplete();
        $this->ajax_success_response('Successfully Vendor data updated!', array(), 'Yes');
        return;
    }
