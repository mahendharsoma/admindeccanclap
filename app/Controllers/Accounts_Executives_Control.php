...
class Accounts_Executives_Control extends Default_Controller
{
	...
	public function index($seg1 = false)
	{
        ...
        if(!empty($enc_parent_id) && !empty($parent_id)){
            ...
            $accounts_executives_info = $this->user_library->get_user_details_by_user_roles_and_user_id($user_roles, $parent_id);
            if($accounts_executives_info === false)
            {
                log_message('error', __METHOD__.' error while getting user data for roles: '.implode(', ', $user_roles));
                $this->set_flash_error('Unable to get users, please try later');
                return redirect()->to('dashboard');
            }
            $data['accounts_executives_data'] = $accounts_executives_info;
        }

	    return $this->generateView('content_panel/User_Management/accounts_executives_view',$data);
	}

    public function ajax_add_accounts_executive()
    {
       // --------------------- VALIDATION ---------------------
        $rules = [
        ...
        $this->ajax_success_response('Successfully Created Accounts Executive!', array(), 'Yes');
        return;
    }

    public function ajax_get_edit_accounts_executive_details()
    {
    ...
    public function ajax_update_accounts_executive()
    {
       $rules = [
            'user_name'        => ['label' => 'User Name', 'rules' => 'required'],
            'email_id'         => ['label' => 'Email ID', 'rules' => 'required'],
        ...
        ---------------------------------------------------------- */
        $this->db->transComplete();
        $this->ajax_success_response('Successfully Accounts Executive data updated!', array(), 'Yes');
        return;
    }
