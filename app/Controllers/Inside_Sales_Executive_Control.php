...
class Inside_Sales_Executive_Control extends Default_Controller
{
    ...
    public function index($seg1 = false, $seg2 = false)
	{
        ...
        if(!empty($enc_parent_id) && !empty($parent_id)){
            ...
            if(!empty($sales_manager_id) && !empty($sales_manager_id))
            {
                ...
                $inside_sales_executives_info = $this->user_library->get_user_details_by_user_roles_and_user_id($user_roles, $sales_manager_id);
                if($inside_sales_executives_info === false)
                {
                    log_message('error', __METHOD__.' error while getting user data for roles: '.implode(', ', $user_roles));
                    $this->set_flash_error('Unable to get users, please try later');
                    return redirect()->to('dashboard');
                }
                $data['inside_sales_executives_data'] = $inside_sales_executives_info;
            }
        }

	    return $this->generateView('content_panel/User_Management/inside_sales_executives_view',$data);
	}

    public function ajax_add_inside_sales_executive()
    {
       // --------------------- VALIDATION ---------------------
        $rules = [
        ...
        $this->ajax_success_response('Successfully Created Inside Sales Executive!', array(), 'Yes');
        return;
    }

    public function ajax_get_edit_inside_sales_executive_details()
    {
    ...
    public function ajax_update_inside_sales_executive()
    {
       $rules = [
            'user_name'        => ['label' => 'User Name', 'rules' => 'required'],
            'email_id'         => ['label' => 'Email ID', 'rules' => 'required'],
        ...
        ---------------------------------------------------------- */
        $this->db->transComplete();
        $this->ajax_success_response('Successfully Inside Sales Executive data updated!', array(), 'Yes');
        return;
    }
